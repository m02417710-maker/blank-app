"""EGX Pro v32 — ML + Backtest Engine (مُصحَّح بالكامل — 19 إصلاح)"""
import pandas as pd
import numpy as np
import logging
...
from egx_engine import ML_TARGET_HORIZON_DAYS, ML_TARGET_THRESHOLD

# ✅ FIX #4: تصحيح هيكل العمولات الفعلي في EGX
# المصدر: لائحة رسوم البورصة المصرية — رسوم السوق 3 bps + دمغة 12.5 bps + حفظ 3 bps
EGX_COMMISSION=0.0003; EGX_STAMP=0.00125; EGX_CUSTODY=0.0003
EGX_TOTAL_COST=EGX_COMMISSION+EGX_STAMP+EGX_CUSTODY   # = 0.00185 (18.5 bps) بدلاً من 0.00265 سابقاً
...
class EGXMLPredictor:
    ...
    def _build_features(self, df):
        ...
        # ✅ threshold 0.5% بدلاً من 2% لإصلاح class imbalance (راجع ML_TARGET_* في egx_engine)
        feat['target']=(c.pct_change(ML_TARGET_HORIZON_DAYS).shift(-ML_TARGET_HORIZON_DAYS)>ML_TARGET_THRESHOLD).astype(int)
        return feat.dropna()
    ...
    def train_walk_forward(self, df, n_splits=5):
        ...
        for fi,(tr_i,te_i) in enumerate(tscv.split(X)):
            ...
            except ValueError: auc=0.5  # يحدث عندما يحتوي fold على فئة واحدة فقط
...
def _run(df,strategy,cap=100_000):
    ...
    def g(col,i,d=0):
        ...
        except (IndexError, KeyError, ValueError, TypeError): return d
    for i in range(1,len(df)-1):
        ...
        if buy and pos==0 and capital>MIN_TRADE_EGP:
            ...
            if sh>0:
                # ✅ FIX #1: cp يحتوي على EGX_TOTAL_COST+EGX_SLIPPAGE — لا نضيفها مرة ثانية
                capital-=sh*cp
                pos,ep,ed=sh,cp,df.index[i]
        elif sell and pos>0:
            sp=_cost(price,'sell'); pr=pos*sp
            # ✅ FIX #2: sp يحتوي على خصم EGX_TOTAL_COST+EGX_SLIPPAGE — لا نخصم tc منفصلة
            pnl=pr-pos*ep; pct=pnl/(pos*ep) if ep>0 else 0
            tc=pos*ep*EGX_TOTAL_COST  # للسجل فقط (تكلفة الدخول، موثقة في Trade)
            capital+=pr
    ...
    if pos>0:
        ...
        # ✅ FIX #2: sp يحتوي على التكاليف — لا نخصم tc منفصلة
        pnl=pr-pos*ep; pct=pnl/(pos*ep) if ep>0 else 0
        tc=pos*ep*EGX_TOTAL_COST  # للسجل فقط
        capital+=pr
...
class PortfolioManager:
    ...
    def calc_position_size(self,price,wr,aw,al,max_risk=0.02):
        # ✅ FIX #3: ضمان أن al (avg_loss) موجب دائماً بغض النظر عن الإشارة
        al=abs(al)
...
def dca_simulation(df: pd.DataFrame, monthly_amount: float, months: int) -> Dict:
    ...
    c = df['close']
    # ✅ FIX #4: استخدام تواريخ فعلية من الـ index بدلاً من step ثابت قد يصطدم بأيام عطلة
    # نختار أول يوم تداول من كل شهر من البيانات المتاحة
    df_copy = df[['close']].copy()
    df_copy['year_month'] = df_copy.index.to_period('M')
    monthly_indices = []
    for _, grp in df_copy.groupby('year_month', sort=True):
        monthly_indices.append(grp.index[0])  # أول يوم تداول في الشهر
    monthly_indices = monthly_indices[:months]
    ...
    for m_idx, dt in enumerate(monthly_indices):
    ...
    first_price = float(c.loc[monthly_indices[0]]) * (1 + EGX_TOTAL_COST + EGX_SLIPPAGE)
