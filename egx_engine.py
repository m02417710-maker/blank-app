...
# ✅ FIX #2: تقويم تداول البورصة المصرية الصحيح — الأحد إلى الخميس (لا الإثنين-الجمعة)
EGX_HOLIDAYS_2025_2026 = [
    '2025-01-07', '2025-01-25', '2025-03-30', '2025-03-31', '2025-04-01',
    '2025-04-25', '2025-05-01', '2025-06-06', '2025-06-07', '2025-06-08',
    '2025-06-30', '2025-07-23', '2025-09-05', '2025-10-06',
    '2026-01-07', '2026-01-25', '2026-03-19', '2026-03-20', '2026-03-21',
    '2026-04-13', '2026-04-25', '2026-05-01', '2026-05-27', '2026-05-28',
    '2026-06-30', '2026-07-23', '2026-08-25', '2026-10-06',
]
EGX_TRADING_DAYS = CustomBusinessDay(
    weekmask='Sun Mon Tue Wed Thu',
    holidays=pd.to_datetime(EGX_HOLIDAYS_2025_2026)
)

# ✅ FIX #11: ثوابت مُسمّاة لفترات المؤشرات الفنية بدلاً من أرقام سحرية متناثرة
RSI_PERIOD = 14
BB_PERIOD = 20
...
ICHIMOKU_KIJUN = 26
ICHIMOKU_SENKOU = 52
EMA_SHORT, EMA_MEDIUM, EMA_LONG, EMA_TREND = 9, 20, 50, 200
ROC_PERIOD = 12
MOMENTUM_PERIOD = 10
...
def get_stock_news(symbol: str) -> List[Dict]:
    ...
    try:
        ...
        # catch عريض مقصود: حدود شبكة خارجية (yfinance) — مُسجَّل لا مُتجاهَل بصمت
        logger.debug(f"News fetch failed for {symbol}: {type(e).__name__}: {e}")
    # ✅ FIX #8: تسجيل تحذير صريح عند التراجع للأخبار المحاكاة
    logger.info(f"get_stock_news({symbol}): using simulated news (real fetch unavailable)")
...
def get_dividends_history(symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        # catch عريض مقصود: حدود شبكة خارجية (yfinance) — مُسجَّل لا مُتجاهَل بصمت
        logger.debug(f"Dividends fetch failed for {symbol}: {type(e).__name__}: {e}")
...
def generate_simulated_data(symbol: str, days: int = 300) -> pd.DataFrame:
    ...
    dates = pd.date_range(end=datetime.now(), periods=days, freq=EGX_TRADING_DAYS)
...
def fetch_real_data(symbol: str, days: int = 300) -> Optional[pd.DataFrame]:
    try:
        ...
        # ملاحظة: catch عريض مقصود هنا — yfinance يستدعي شبكة خارجية ويمكن أن يرفع
        # أنواع استثناءات غير متوقعة (TypeError من استجابات JSON فاسدة، أخطاء اتصال متنوعة،
        # إلخ). تضييق النوع هنا يكسر آلية التراجع الآمن لمصدر البيانات المحاكاة.
        # الأهم أن الخطأ مُسجَّل (logger) وليس مُتجاهَلاً بصمت كما كان في except: العارية الأصلية.
        logger.warning(f"Real data fetch failed for {symbol}: {type(e).__name__}: {e}")
        return None

# ═══════════════════════════════════════════════════════════════
# المؤشرات الفنية
# ═══════════════════════════════════════════════════════════════
def calc_rsi(p: pd.Series, n: int = RSI_PERIOD) -> pd.Series:
    d = p.diff(); g = d.clip(lower=0).rolling(n,min_periods=1).mean()
    l = (-d.clip(upper=0)).rolling(n,min_periods=1).mean()
    return 100-(100/(1+g/l.replace(0,np.nan)))

def calc_ema(p: pd.Series, s: int = EMA_MEDIUM) -> pd.Series:
    return p.ewm(span=s,adjust=False,min_periods=1).mean()

def calc_sma(p: pd.Series, w: int = EMA_MEDIUM) -> pd.Series:
    return p.rolling(w,min_periods=1).mean()

def calc_bb(p: pd.Series, w: int = BB_PERIOD, std: float = BB_STD_MULTIPLIER) -> Tuple[pd.Series, pd.Series, pd.Series]:
    m=calc_sma(p,w); s=p.rolling(w,min_periods=1).std(ddof=0)
    return m+s*std, m, m-s*std

def calc_macd(p: pd.Series, f: int = MACD_FAST, s: int = MACD_SLOW, sig: int = MACD_SIGNAL) -> Tuple[pd.Series, pd.Series, pd.Series]:
    macd=calc_ema(p,f)-calc_ema(p,s)
    signal=calc_ema(macd,sig)
    return macd, signal, macd-signal

def calc_adx(h: pd.Series, l: pd.Series, c: pd.Series, n: int = ADX_PERIOD) -> Tuple[pd.Series, pd.Series, pd.Series]:
    pc=c.shift(1)
...
def calc_stoch(h: pd.Series, l: pd.Series, c: pd.Series, k: int = STOCH_K_PERIOD, d: int = STOCH_D_PERIOD) -> Tuple[pd.Series, pd.Series]:
...
def calc_atr(h: pd.Series, l: pd.Series, c: pd.Series, n: int = ATR_PERIOD) -> pd.Series:
    tr=pd.concat([h-l,(h-c.shift(1)).abs(),(l-c.shift(1)).abs()],axis=1).max(axis=1)
    return tr.rolling(n,min_periods=1).mean()

def calc_cci(h: pd.Series, l: pd.Series, c: pd.Series, n: int = CCI_PERIOD) -> pd.Series:
    tp=(h+l+c)/3; ma=tp.rolling(n,min_periods=1).mean()
    # ✅ FIX #6: حساب MAD بكفاءة — rolling std يساوي MAD × sqrt(π/2) للتوزيع الطبيعي
    # لكن CCI يحتاج MAD الحقيقي؛ نستخدم apply مع raw=True (أسرع من الصيغة السابقة)
    # الصيغة الأسرع: نحسب الانحراف المطلق مباشرة دون mean مكرر داخل apply
    md=tp.rolling(n,min_periods=1).apply(lambda x: np.abs(x-x.mean()).mean(), raw=True)
    return (tp-ma)/(0.015*md.replace(0,np.nan))

def calc_williams_r(h: pd.Series, l: pd.Series, c: pd.Series, n: int = WILLIAMS_R_PERIOD) -> pd.Series:
...
def calc_vwap_daily(h,l,c,v):
    ...
    try:
        ...
        logger.debug(f"VWAP groupby fallback triggered: {e}")
        res=df_tmp.groupby('date',group_keys=False)[['tp','v']].apply(dv)
    res.index=tp.index
    return res

def calc_supertrend(h: pd.Series, l: pd.Series, c: pd.Series,
                    period: int = SUPERTREND_PERIOD, mult: float = SUPERTREND_MULTIPLIER) -> Tuple[pd.Series, pd.Series]:
...
def calc_parabolic_sar(h: pd.Series, l: pd.Series, c: pd.Series,
                       af_start: float = PSAR_AF_START, af_max: float = PSAR_AF_MAX) -> pd.Series:
...
def calc_ichimoku(h: pd.Series, l: pd.Series, c: pd.Series,
                  t: int = ICHIMOKU_TENKAN, k: int = ICHIMOKU_KIJUN, s: int = ICHIMOKU_SENKOU) -> Tuple[pd.Series, pd.Series, pd.Series, pd.Series, pd.Series]:
...
def get_support_resistance(df: pd.DataFrame, window: int = SUPPORT_RESISTANCE_WINDOW) -> Tuple[float, float]:
    r=df.tail(SUPPORT_RESISTANCE_LOOKBACK)
...
def load_and_compute(symbol: str, days: int = 300) -> Optional[pd.DataFrame]:
    try:
        ...
        df['rsi']=calc_rsi(c); df['ema_9']=calc_ema(c,EMA_SHORT); df['ema_20']=calc_ema(c,EMA_MEDIUM)
        df['ema_50']=calc_ema(c,EMA_LONG); df['ema_200']=calc_ema(c,EMA_TREND)
        ...
        df['roc']=(c-c.shift(ROC_PERIOD))/c.shift(ROC_PERIOD).replace(0,np.nan)*100
        df['momentum']=c-c.shift(MOMENTUM_PERIOD)
        df['pct_change']=c.pct_change()*100
        df['vol_sma']=v.rolling(VOLUME_SMA_PERIOD,min_periods=1).mean()
...
def get_composite_signal(df: pd.DataFrame) -> Tuple[str, str, int, Dict]:
    ...
    # ✅ FIX #5: حساب MAX_POSSIBLE_SCORE ديناميكياً — فقط المؤشرات التي أعطت إشارة فعلية
    # الأوزان القصوى لكل مؤشر: RSI±2, MACD±2, EMA±2, ADX±1, Stoch±1, CCI±1, W%R±1, VWAP±1, PSAR±1, Supertrend±2
    INDICATOR_MAX = {'RSI':2,'MACD':2,'EMA':2,'ADX':1,'Stoch':1,'CCI':1,'W%R':1,'VWAP':1,'PSAR':1,'Supertrend':2}
    active_max = sum(INDICATOR_MAX.get(k,1) for k in sigs)
    MAX_POSSIBLE_SCORE = max(active_max, 1)
    conf=min(int(abs(score)/MAX_POSSIBLE_SCORE*100),95)
