...
class TechnicalAnalysisEngine:
    ...
    def _compute_trend_classification(self, df: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
        ...
        for i in range(len(df)):
            ...
            ema200 = df["ema_200"].iloc[i] if "ema_200" in df.columns else None
            adx = df["adx"].iloc[i]

            score = 0
            if close > ema9: score += 15
            if ema9 > ema21: score += 15
            if ema21 > ema50: score += 15
            if ema200 and ema50 > ema200: score += 15
            if adx > 25: score += 20
            if df["plus_di"].iloc[i] > df["minus_di"].iloc[i]: score += 10

            if score >= 70:
                trend.iloc[i] = "Strong Up"
                strength.iloc[i] = score
            elif score >= 50:
                trend.iloc[i] = "Up"
                strength.iloc[i] = score
            elif score <= 30:
                trend.iloc[i] = "Strong Down"
                strength.iloc[i] = 100 - score
            elif score <= 50:
                trend.iloc[i] = "Down"
                strength.iloc[i] = 100 - score
            else:
