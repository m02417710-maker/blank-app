"""
EGX Pro Ultimate v32 — الكود النهائي المحسّن
✅ جميع الأخطاء مصحّحة  ✅ أداء محسّن  ✅ أمان مُحكَم
✅ 200+ شركة  ✅ أخبار وتوزيعات وتوقعات  ✅ ذكاء اصطناعي
"""
...
def safe_plotly_chart(fig, **kwargs):
    """✅ تغليف st.plotly_chart بـ try-except لمنع تعطل التطبيق"""
    try:
...
def init_state():
    """✅ تهيئة حالة الجلسة بقيم افتراضية آمنة"""
    defaults = {
        'watchlist': ['COMI', 'TMGH', 'ETEL', 'FWRY', 'SWDY', 'ECIG', 'SKPC'],
        'alerts': [],
        'capital': 100_000.0,
        'page': 'home'
    }
...
def get_ai_analyzer():
    """✅ تخزين مؤقت لمحلل الذكاء الاصطناعي"""
    return AIAnalyzer()
...
def _color_backtest(df_style):
    """✅ تلوين جدول الباكتست — مستوى الوحدة"""
    def _cell_color(val):
        if isinstance(val, str) and '%' in val:
            try:
                ...
                pass
        return ''
    return df_style.map(_cell_color)


def _validate_df(df, min_rows=2):
    """✅ FIX #1: التحقق من صحة DataFrame قبل الاستخدام"""
    if df is None:
        return False
...
def _safe_price(df):
    """✅ FIX #2: جلب السعر بأمان مع التحقق"""
    if not _validate_df(df, min_rows=1):
        return None
    try:
...
def _safe_pct_change(df):
    """✅ FIX #3: جلب نسبة التغيير بأمان"""
    if not _validate_df(df, min_rows=2):
        return 0.0
    try:
        return float(df['pct_change'].iloc[-1])
    except (IndexError, KeyError, ValueError):
        return 0.0
...
def plot_main(df, symbol, indicators):
    ...
    if 'EMA' in indicators:
        for col, clr, lbl in [
            ('ema_9', '#f59e0b', 'EMA 9'),
            ('ema_20', '#3b82f6', 'EMA 20'),
            ('ema_50', '#8b5cf6', 'EMA 50'),
            ('ema_200', '#ec4899', 'EMA 200')
        ]:
    ...
    if 'VWAP' in indicators and 'vwap' in df.columns:
        fig.add_trace(go.Scatter(
            x=df.index, y=df['vwap'], name='VWAP يومي',
    ...
    if 'Supertrend' in indicators and 'supertrend' in df.columns:
        ...
        fig.add_trace(go.Scatter(
            x=df.index, y=up, name='ST شراء',
            line=dict(color='#10b981', width=2.5)
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=df.index, y=dn, name='ST بيع',
            line=dict(color='#ef4444', width=2.5)
        ), row=1, col=1)

    if 'Ichimoku' in indicators:
        for col, clr, lbl in [
            ('ich_tenkan', '#f59e0b', 'Tenkan'),
            ('ich_kijun', '#3b82f6', 'Kijun')
        ]:
            if col in df.columns:
                fig.add_trace(go.Scatter(
                    x=df.index, y=df[col], name=lbl,
        ...
        if 'ich_senkou_a' in df.columns and 'ich_senkou_b' in df.columns:
            fig.add_trace(go.Scatter(
                x=df.index, y=df['ich_senkou_a'], name='Senkou A',
                line=dict(color='#10b981', width=1), opacity=0.5
            ), row=1, col=1)
            fig.add_trace(go.Scatter(
                x=df.index, y=df['ich_senkou_b'], name='Senkou B',
    ...
    if 'PSAR' in indicators and 'psar' in df.columns:
        fig.add_trace(go.Scatter(
            ...
            mode='markers', marker=dict(size=3, color='#a78bfa')
        ), row=1, col=1)
    ...
    if 'MACD' in indicators and 'macd' in df.columns:
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=clrs_hist, opacity=0.7
        ), row=cr, col=1)
        cr += 1
    ...
    if 'Volume' in indicators and 'volume' in df.columns:
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=vclrs, opacity=0.8
        ), row=cr, col=1)
...
def page_home():
    st.markdown(
        ...
        '<div class="page-title">📊 لوحة السوق الرئيسية</div>'
        '<div class="page-sub">بورصة الأوراق المالية المصرية — تحديث فوري</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    for row_start in range(0, len(display_syms), 5):
        ...
        for col, sym in zip(cols, display_syms[row_start:row_start + 5]):
            ...
            if _validate_df(df, min_rows=2):
                price = _safe_price(df)
                chg = _safe_pct_change(df)
                vol = int(df['volume'].iloc[-1])
                src = df.get('data_source', pd.Series(['?'])).iloc[-1]
                clr = clr_sign(chg)
                arr = "▲" if chg >= 0 else "▼"
                with col:
                    st.markdown(f"""
                    <div class="mcard">
                        ...
                        <span class="{'badge-real' if src == 'real' else 'badge-sim'}">
                            {'🟢 حقيقي' if src == 'real' else '🟡 محاكاة'}
                        </span>
                    </div>""", unsafe_allow_html=True)
            else:
                with col:
                    st.warning(f"⚠️ لا بيانات: {sym}")
    ...
    if wl:
        ...
        with st.spinner("جاري تحليل القائمة..."):
            for sym in wl:
                df = cached_load(sym)
                if _validate_df(df, min_rows=5):
                    chg1 = _safe_pct_change(df)
                    chg5 = float(df['close'].pct_change(5).iloc[-1] * 100) if len(df) >= 5 else 0.0
                    sig, em, conf, _ = get_composite_signal(df)
                    perf.append({
                        'رمز': sym,
                        'الشركة': EGXDatabase.STOCKS.get(sym, {}).get('name', '')[:20],
                        'اليوم': chg1,
                        '5 أيام': chg5,
                        'إشارة': f"{em} {sig}",
                        'ثقة': f"{conf}%"
        ...
        if perf:
            ...
            with c1:
                ...
                for _, r in df_p.head(3).iterrows():
                    st.success(
                        f"**{r['رمز']}** {r['الشركة']} | "
                        f"اليوم: {r['اليوم']:+.2f}% | 5 أيام: {r['5 أيام']:+.2f}%"
                    )
            with c2:
...
def page_analysis():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">🔍 تحليل السهم</div>'
        '<div class="page-sub">تحليل فني شامل مع جميع المؤشرات</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    with c1:
        sym = st.selectbox("اختر السهم", all_syms, index=all_syms.index('COMI') if 'COMI' in all_syms else 0)
    with c2:
    ...
    with c3:
        ...
        if st.button("➕ أضف للمراقبة"):
            if sym not in st.session_state['watchlist']:
                ...
                st.success(f"✅ تمت إضافة {sym}")

    with st.spinner(f"جاري تحليل {sym}..."):
        df = cached_load(sym, days)

    # ✅ FIX #4: التحقق الشامل من صحة البيانات
    if not _validate_df(df):
        st.error("⚠️ لا بيانات كافية لهذا السهم. جرب سهماً آخر.")
        return
    ...
    <div style="background:linear-gradient(135deg,#1a1f2e,#2d3748);border-radius:14px;
        ...
        <div>
            ...
            <div style="color:#64748b;font-size:13px;margin-top:4px">
                {sym} &nbsp;&middot;&nbsp; {info.get('sector', '')} &nbsp;&middot;&nbsp;
                <span class="{'badge-real' if src == 'real' else 'badge-sim'}">
                    {'🟢 بيانات حقيقية' if src == 'real' else '🟡 محاكاة GARCH'}
                </span>
            </div>
        </div>
    ...
    # ── مقاييس سريعة ──
    h52 = float(df['high'].tail(252).max()) if len(df) >= 252 else float(df['high'].max())
    l52 = float(df['low'].tail(252).min()) if len(df) >= 252 else float(df['low'].min())
    avg_vol = int(df['volume'].tail(20).mean()) if len(df) >= 20 else int(df['volume'].mean())
    rsi_v = safe_last(df['rsi'], 50)
    atr_v = safe_last(df['atr'])
    vol20 = safe_last(df['volatility_20d'], 0) * 100
    pe = info.get('pe', 0)
    div_y = info.get('div_yield', 0)
    ...
    # ── تبويبات التحليل ──
    tabs = st.tabs([
        "📈 الرسم البياني", "🎯 الإشارات", "🕯️ الشموع",
        "📐 دعم ومقاومة", "📰 الأخبار", "💰 التوزيعات", "🎯 التوقعات"
    ])

    with tabs[0]:
        ind_opts = ['EMA', 'Bollinger', 'VWAP', 'Supertrend', 'Ichimoku', 'PSAR', 'RSI', 'MACD', 'Volume']
        show_ind = st.multiselect(
            "المؤشرات المعروضة", ind_opts,
    ...
    with tabs[1]:
        ...
        for name, desc in sigs.items():
            is_buy = any(k in desc for k in ["شراء", "صاعد", "✅", "فوق"])
            is_sell = any(k in desc for k in ["بيع", "هابط", "❌", "تحت"])
    ...
    with tabs[6]:
        ...
        if targets:
            ...
            c1.metric("🟢 هدف صاعد", f"{targets['bull_target']:.2f}", f"+{targets['upside_pct']:.1f}%")
            c2.metric("📊 هدف أساسي", f"{targets['base_target']:.2f}")
            c3.metric("🔴 هدف هابط", f"{targets['bear_target']:.2f}", f"{targets['downside_pct']:.1f}%")
            ...
            for val, clr2, lbl2 in [
                ...
                fig2.add_hline(
                    y=val, line_color=clr2, line_dash='dash', line_width=2,
...
def page_supertrend():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">⚡ محلل Supertrend</div>'
        '<div class="page-sub">أقوى مؤشر اتجاه في السوق المصري</div>'
        '</div>',
        unsafe_allow_html=True
    )
    c1, c2, c3 = st.columns(3)
    with c1:
        sym = st.selectbox("السهم", sorted(EGXDatabase.STOCKS.keys()), key='st_sym')
    with c2:
    ...
    if not _validate_df(df):
        st.error("لا بيانات كافية لهذا السهم")
        return
    ...
    cur_price = _safe_price(df)
    cur_st = float(df['st_c'].iloc[-1]) if 'st_c' in df.columns else cur_price
    cur_dir = int(df['st_d'].iloc[-1]) if 'st_d' in df.columns else 0
...
def page_backtest():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">🧪 الباكتست التاريخي</div>'
        '<div class="page-sub">اختبر 10 استراتيجيات على بيانات حقيقية مع محاكاة عمولات EGX</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    with bt_tabs[0]:
        ...
        if st.button("🚀 تشغيل الباكتست الآن", type="primary"):
            ...
            if results:
                ...
                st.info(
                    f"🏆 أفضل استراتيجية: **{best.strategy}** | "
                    f"عائد {best.total_return * 100:.1f}% | "
                ...
                fig = go.Figure()
                clrs_list = ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444',
                ...
                valid_strats = [r.strategy for r in results if r.total_trades > 0]
                if valid_strats:
                    sel_s = st.selectbox("تفاصيل الصفقات", valid_strats)
                    sel_r = next((r for r in results if r.strategy == sel_s), None)
                    if sel_r and sel_r.trades:
                        trades_df = pd.DataFrame([
                            {
                                'دخول': str(t.entry_date)[:10],
                                'خروج': str(t.exit_date)[:10],
                                'سعر الدخول': f"{t.entry_price:.2f}",
                                'سعر الخروج': f"{t.exit_price:.2f}",
                                'أسهم': t.shares,
                                'ربح/خسارة EGP': f"{t.pnl:+.0f}",
    ...
    with bt_tabs[1]:
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            dca_sym = st.selectbox("السهم", sorted(EGXDatabase.STOCKS.keys()), key='dca_sym')
        with c2:
            monthly = st.number_input("المبلغ الشهري (EGP)", 500, 1_000_000, 5_000, 500)
        with c3:
            months = st.slider("عدد الشهور", 3, 36, 12)

        if st.button("🚀 ابدأ محاكاة DCA", type="primary"):
            with st.spinner("جاري محاكاة DCA..."):
                df_dca = cached_load(dca_sym, max(months * 22, 300))
                if not _validate_df(df_dca):
                    st.error("لا بيانات")
                    return
            ...
            if 'error' in result:
                ...
                fig = go.Figure()
                fig.add_trace(go.Scatter(
                    x=df_m['month'], y=df_m['total_invested'],
                    name='إجمالي المستثمر', line=dict(color='#64748b', width=2, dash='dash')
                ))
                fig.add_trace(go.Scatter(
                    x=df_m['month'], y=df_m['current_value'],
                    name='القيمة الحالية', line=dict(color='#10b981', width=2.5),
                    fill='tonexty', fillcolor='rgba(16,185,129,0.08)'
                ))
...
def _train_ml_cached(symbol: str, days: int, n_splits: int):
    """✅ تخزين مؤقت للنموذج المُدرَّب — يمنع إعادة التدريب في كل نقرة"""
    try:
...
def page_ml():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">🤖 تحليل الذكاء الاصطناعي</div>'
        '<div class="page-sub">Walk-Forward Validation — تدريب على الماضي، اختبار على المستقبل</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    with c2:
        days = st.slider("الفترة (يوم)", 100, 500, 300, 50, key='ml_days')
    ...
    if train_clicked:
        ...
        if pred is None or 'error' in metrics:
            st.error(f"خطأ: {metrics.get('error', 'فشل غير معروف')}")
            return
...
def page_portfolio():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">💼 إدارة المحفظة</div>'
        '<div class="page-sub">Kelly Criterion + وقف الخسارة الديناميكي — مبني على نتائج باكتست فعلية</div>'
        '</div>',
        unsafe_allow_html=True
    )

    st.session_state['capital'] = st.number_input(
        "رأس المال الكلي (EGP)", 10_000.0, 10_000_000.0,
    ...
    strategy_for_sizing = st.selectbox(
        "استراتيجية حساب حجم المركز", ALL_STRATEGIES,
        index=ALL_STRATEGIES.index('Multi-Signal') if 'Multi-Signal' in ALL_STRATEGIES else 0,
    ...
    # ✅ حساب win_rate/avg_win/avg_loss من باكتست فعلي
...
def page_watchlist():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">👁️ قائمة المراقبة</div>'
        '<div class="page-sub">تابع أسهمك المفضلة في مكان واحد</div>'
        '</div>',
        unsafe_allow_html=True
    )

    new_wl = st.multiselect(
        "اختر الأسهم", sorted(EGXDatabase.STOCKS.keys()),
    ...
    with st.spinner("جاري التحليل..."):
        ...
        for sym in new_wl:
            ...
            rows.append({
                'رمز': sym,
                'الشركة': EGXDatabase.STOCKS.get(sym, {}).get('name', '')[:22],
    ...
    if rows:
        ...
        st.warning("لا توجد بيانات للأسهم المحددة")


def page_screener():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">🔎 فلترة الأسهم</div>'
        '<div class="page-sub">ابحث عن أفضل الفرص بمعايير متعددة</div>'
        '</div>',
        unsafe_allow_html=True
    )
    c1, c2, c3 = st.columns(3)
    with c1:
        sector = st.selectbox("القطاع", ['الكل'] + sorted(EGXDatabase.SECTORS.keys()))
    with c2:
        ...
        for idx, sym in enumerate(syms):
            ...
            if not _validate_df(df, min_rows=1):
                ...
                if not (rsi_range[0] <= rsi <= rsi_range[1]):
                    passed = False
                elif info.get('div_yield', 0) < min_div:
                    passed = False
                elif supertrend_only and std != 1:
                    passed = False
                else:
                    sig, em, conf, _ = get_composite_signal(df)
...
def page_sector():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">🏭 تحليل القطاعات</div>'
        '<div class="page-sub">مقارنة أداء القطاعات ورسم خريطة السوق</div>'
        '</div>',
        unsafe_allow_html=True
    )
    sel = st.multiselect(
        "اختر القطاعات", sorted(EGXDatabase.SECTORS.keys()),
    ...
    with st.spinner("جاري تحليل القطاعات..."):
        for sec in sel:
            ...
            for sym in syms:
                ...
                if not _validate_df(df, min_rows=1):
                    continue
                chgs.append(_safe_pct_change(df))
                rsis.append(safe_last(df['rsi'], 50))
                s, _, _, _ = get_composite_signal(df)
                sigs.append(s)
            if chgs:
                ...
                data.append({
                    'القطاع': sec,
                    'أسهم': len(syms),
                    'متوسط التغيير%': np.mean(chgs),
                    'متوسط RSI': np.mean(rsis),
                    'إشارات شراء%': buy_p
                })

    if not data:
        st.info("لا توجد بيانات كافية للقطاعات المحددة")
        return
...
def page_alerts():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">🔔 نظام التنبيهات</div>'
        '<div class="page-sub">راقب شروطك تلقائياً في كل تحديث</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    with st.form("add_alert_form"):
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            atype = st.selectbox("النوع", [
                'سعر أعلى من', 'سعر أقل من', 'RSI تشبع شراء (>70)',
                'RSI تشبع بيع (<30)', 'Supertrend شراء', 'Supertrend بيع'
            ])
        with c2:
            thresh = st.number_input("القيمة", value=price * 1.05, format="%.2f")
        with c3:
            note = st.text_input("ملاحظة", "")
        if st.form_submit_button("➕ إضافة التنبيه", type="primary"):
            st.session_state['alerts'].append({
                'sym': sym, 'type': atype, 'threshold': thresh,
    ...
    if st.session_state['alerts']:
        ...
        for al in st.session_state['alerts']:
            ...
            if not _validate_df(a_df, min_rows=1):
                continue
            ap = _safe_price(a_df)
            ar = safe_last(a_df['rsi'], 50)
            asd = safe_last(a_df['supertrend_dir'], 0) if 'supertrend_dir' in a_df.columns else 0

            trig = (
                (al['type'] == 'سعر أعلى من' and ap > al['threshold']) or
            ...
            bg = (
                'background:#052e16;border-color:#10b981;color:#6ee7b7' if trig
                else 'background:#1a1f2e;border-color:#3d4a63;color:#94a3b8'
            )
            st.markdown(f"""
...
def page_compare():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">⚖️ مقارنة الأسهم</div>'
        '<div class="page-sub">قارن أداء حتى 5 أسهم معاً</div>'
        '</div>',
        unsafe_allow_html=True
    )
    selected = st.multiselect(
        "اختر حتى 5 أسهم", sorted(EGXDatabase.STOCKS.keys()),
        default=['COMI', 'TMGH', 'ETEL']
    )[:5]
    if len(selected) < 2:
        st.warning("اختر سهمين على الأقل")
        return
    ...
    fig = go.Figure()
    rows = []
    clrs_list = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6']
    for sym, cl in zip(selected, clrs_list):
        ...
        rsi = safe_last(df['rsi'], 50)
        vol = safe_last(df['volatility_20d'], 0) * 100
        sig, _, conf, _ = get_composite_signal(df)
        fig.add_trace(go.Scatter(
            x=df.index, y=norm, name=f"{sym} ({chg:+.1f}%)",
...
def page_fibonacci():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">🌀 تحليل فيبوناتشي</div>'
        '<div class="page-sub">مستويات الدعم والمقاومة الفنية</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    c1, c2 = st.columns(2)
    with c1:
        fp = st.slider("فترة الحساب (يوم)", 20, 252, 60)
    with c2:
        ext = st.checkbox("مستويات التمديد", True)
    ...
    fib_clrs = {
        '0% (دعم)': '#10b981', '23.6%': '#84cc16', '38.2%': '#f59e0b',
        '50%': '#f97316', '61.8%': '#ef4444', '78.6%': '#dc2626',
    ...
    for lvl, val in fib.items():
        ...
        fig.add_hline(
            y=val, line_color=fib_clrs.get(lvl, '#64748b'),
            line_dash='solid' if near else 'dash',
            line_width=2 if near else 1,
            opacity=0.9 if near else 0.6,
            annotation_text=f"{lvl}: {val:.2f}{'  ⭐' if near else ''}",
            annotation_position="right"
        )
    fig.add_hline(
        y=price, line_color='#ffffff', line_width=2, line_dash='dot',
    ...
    st.info(
        f"📍 أقرب مستوى: **{near_lvl[0]}** عند **{near_lvl[1]:.2f} EGP** "
        f"(بُعد {abs(price - near_lvl[1]) / price * 100:.2f}%)"
    )
...
def page_ai_insights():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">🧠 التحليل الذكي (Gemini AI)</div>'
        '<div class="page-sub">شرح المؤشرات وتوليد استراتيجيات استثمار مخصصة بالعربية</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    if not AI.is_available():
        st.warning(
            "⚠️ Gemini API غير مُفعّل حالياً — تُعرض إجابات مبدئية بديلة. "
            "لتفعيل التحليل الكامل، أضف `GEMINI_API_KEY` في `.streamlit/secrets.toml` "
            "أو متغيرات البيئة."
        )
    ...
    with ai_tabs[0]:
        ...
        if st.button("📖 اشرح المؤشر", type="primary"):
            ...
            st.markdown(
                f"""<div style="background:#1a1f2e;border:1px solid #3d4a63;border-radius:12px;
    ...
    with ai_tabs[1]:
        ...
        with c1:
            risk = st.select_slider(
                "مستوى المخاطرة",
                options=["منخفض جداً", "منخفض", "متوسط", "عالي", "عالي جداً"],
                value="متوسط"
            )
...
def page_database():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">📚 قاعدة بيانات EGX</div>'
        '<div class="page-sub">عرض وفلترة تفصيلية لجميع الشركات المدعومة</div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    db_df = pd.DataFrame([
        {
            'الرمز': k,
            'الاسم': v['name'],
            'الاسم بالإنجليزية': v.get('name_en', ''),
            'القطاع': v['sector'],
            'P/E': v.get('pe', 0),
            'EPS': v.get('eps', 0),
            'عائد التوزيعات%': v.get('div_yield', 0),
            'القيمة السوقية (مليون)': v.get('market_cap', 0),
...
def page_about():
    st.markdown(
        '<div class="page-header">'
        '<div class="page-title">ℹ️ عن EGX Pro v32</div>'
        '<div class="page-sub">منصة التحليل الفني الاحترافية لبورصة الأوراق المالية المصرية</div>'
        '</div>',
        unsafe_allow_html=True
    )
    st.markdown("""
    <div style="background:#1a1f2e;border-radius:14px;padding:28px;line-height:2;border:1px solid #3d4a63">
    <h3 style="color:#f59e0b;margin-top:0">🚀 ما الجديد في v32</h3>
    <ul style="color:#e2e8f0">
      <li>✅ <b>نظام تحقق شامل</b> — _validate_df() يمنع جميع أخطاء البيانات الفارغة</li>
      <li>✅ <b>جلب آمن للأسعار</b> — _safe_price() و _safe_pct_change() مع معالجة الاستثناءات</li>
      <li>✅ <b>Logging كامل</b> — تتبع الأخطاء في الإنتاج بسهولة</li>
      <li>✅ <b>أمان XSS محسّن</b> — عدم استخدام مدخلات المستخدم مباشرة في HTML</li>
      <li>✅ <b>معالجة أخطاء ML</b> — try-except حول تدريب النموذج</li>
      <li>✅ <b>حماية من القسمة على صفر</b> — في Supertrend و Kelly Criterion</li>
      ...
      <li>✅ <b>200+ شركة</b> ببيانات: EPS, P/E, توزيعات, market cap</li>
    </ul>
...
def render_sidebar():
    with st.sidebar:
        ...
        for key, (label, _) in PAGES.items():
            is_active = key == cur
            st.button(
                label, key=f"sb_{key}", help=label,
                type="primary" if is_active else "secondary",
                on_click=_navigate_to, args=(key,)
            )

        st.markdown("---")
        n_total = len(EGXDatabase.STOCKS)
        n_sectors = len(EGXDatabase.SECTORS)
        wl_n = len(st.session_state['watchlist'])
        alerts_n = len(st.session_state['alerts'])
        st.markdown(f"""
        <div style="font-size:12px;color:#475569;padding:8px 0;line-height:2">
            🏦 {n_total} شركة · {n_sectors} قطاع<br>
            🏆 EGX30: {len(EGXDatabase.EGX30)} سهم<br>
