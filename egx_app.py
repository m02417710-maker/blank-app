...
def page_ml():
    ...
    if st.button("🧠 تدريب النموذج الآن", type="primary"):
        with st.spinner("جاري التدريب بـ Walk-Forward Validation..."):
            df=cached_load(sym,days)
            if df is None: st.error("لا بيانات"); return
            pred=EGXMLPredictor(); metrics=pred.train_walk_forward(df,n_splits)
        if 'error' in metrics: st.error(f"خطأ: {metrics['error']}"); return
...
def page_portfolio():
    st.markdown('<div class="page-header"><div class="page-title">💼 إدارة المحفظة</div><div class="page-sub">Kelly Criterion + وقف الخسارة الديناميكي</div></div>', unsafe_allow_html=True)
...
def page_screener():
    ...
    if st.button("🔍 تشغيل الفلترة الآن", type="primary"):
        ...
        results_sc=[]; prog=st.progress(0)
        for idx,sym in enumerate(syms):
            df=cached_load(sym)
            if df is None: prog.progress((idx+1)/len(syms)); continue
            rsi=safe_last(df['rsi'],50)
            if not (rsi_range[0]<=rsi<=rsi_range[1]): prog.progress((idx+1)/len(syms)); continue
            info=EGXDatabase.STOCKS.get(sym,{})
            if info.get('div_yield',0)<min_div: prog.progress((idx+1)/len(syms)); continue
            std=safe_last(df['supertrend_dir'],0) if 'supertrend_dir' in df.columns else 0
            if supertrend_only and std!=1: prog.progress((idx+1)/len(syms)); continue
            sig,em,conf,_=get_composite_signal(df)
            if signal_filter!='الكل' and signal_filter not in sig: prog.progress((idx+1)/len(syms)); continue
            price=float(df['close'].iloc[-1]); chg=float(df['pct_change'].iloc[-1])
...
def page_sector():
    ...
    with c1:
        ...
        fig.update_layout(**CHART_LAYOUT,height=400); st.plotly_chart(fig,use_container_width=True)
    with c2:
        ...
        fig2.update_layout(**CHART_LAYOUT,height=400); st.plotly_chart(fig2,use_container_width=True)
